...
# Minimal stub config inheriting TradingConfig (assuming it can be instantiated directly)
class DummyConfig(TradingConfig):
    def __init__(self):
        # Initialize base attributes if needed
...
# Simulate profit reaching 500 (max_pnl_reached)
pos.max_pnl_reached = 500.0
# Current premium that yields exactly 500 profit (entry_price + profit)
current_premium = pos.entry_price + (pos.max_pnl_reached / pos.lot_size)

# Run exit check to trigger trailing stop logic
exit_reason = bot.check_exit_conditions(pos, current_premium, 15000.0, 0, None)

print('Dynamic trailing SL after reaching 500 P&L:', pos.dynamic_trailing_sl)
print('Exit reason (should be None, not STOP_LOSS):', exit_reason)
